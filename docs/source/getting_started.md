...
3. **Install pydar_utils**: pydar_utils is published with PyPA (the python packacing authority), so you can install the latest stable release of pydar_utils using pip. This installs our latest stable release as well as several libraries required for the use of the package's features. pydar_utils currently supports Python versions 3.9. 3.10 and 3.11.

```bash
   pip install pydar_utils
```
